...
namespace WangPluginSav.Util.Pikaedit
{
	public class WonderCard
	{
		...
		public RibbonSet ribbons
		{
			get
			{
				return new RibbonSet(BitConverter.ToUInt16(data, 12));
			}
			set
			{
				Array.Copy(BitConverter.GetBytes(value.data), 0, data, 12, 2);
			}
		}
		...
		public IVSet iv
		{
			get
			{
				return new IVSet(BitConverter.ToUInt32(data, 67));
			}
			set
			{
				Array.Copy(BitConverter.GetBytes(Convert.ToUInt32(value.getIV())), 0, data, 67, 4);
			}
		}
		...
		public MoveSet moveset
		{
			get
			{
				return new MoveSet(new Move(BitConverter.ToUInt16(data, 18), 0, 0), new Move(BitConverter.ToUInt16(data, 20), 0, 0), new Move(BitConverter.ToUInt16(data, 22), 0, 0), new Move(BitConverter.ToUInt16(data, 24), 0, 0));
			}
			set
			{
				Array.Copy(BitConverter.GetBytes(value.move1.move), 0, data, 18, 2);
				Array.Copy(BitConverter.GetBytes(value.move2.move), 0, data, 20, 2);
				Array.Copy(BitConverter.GetBytes(value.move3.move), 0, data, 22, 2);
				Array.Copy(BitConverter.GetBytes(value.move4.move), 0, data, 24, 2);
			}
		}
